...
- **REST controller** (`src/server/rest/controllers`):
  - Guard access with `ensureAuthenticated(context)` when the domain requires auth.
  - Parse and validate params using the shared schema, then delegate straight to the service.
  - Register the path in the controller export so OpenAPI stays current.
...
- **Route handler** (`src/app/api/v1/<entity>/[id]/route.ts`):
  - Compose `createRestHandler` with `IdParamSchema.parse(params)` to fail fast on invalid IDs.
  - Return the controller result directly, letting `RestError` propagate for consistent HTTP codes.
...
3. Wire the REST handler that reuses shared param schemas, enforces auth, and delegates to the service.
