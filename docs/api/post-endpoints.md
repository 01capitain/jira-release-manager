...
- **tRPC router** (`src/server/api/routers`):
  - Wrap the shared schema in `protectedProcedure.input(...).mutation(...)` and call `requireUserId(ctx.session)` so unauthenticated requests short-circuit.
  - Instantiate `ActionHistoryService` to start an action with a descriptive `actionType` and message. Pass the resulting logger into the service and ensure both success and error paths complete the action.
  - Return the service DTO verbatim; do not re-shape the payload.
- **REST controller** (`src/server/rest/controllers`):
  ...
  - Start an `ActionHistoryService` action mirroring the tRPC implementation. Trim inputs again only if the schema cannot express the normalization.
...
- When a tRPC mutation remains available, include integration tests (if applicable) to confirm it mirrors the REST behavior.
...
3. Wire both tRPC and REST endpoints, ensuring authentication and action history tracking.
