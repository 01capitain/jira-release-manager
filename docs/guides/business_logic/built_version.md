...
- On Release creation: An initial Built Version is automatically created with name pattern `{release_version}.{increment}` using increment `0` (e.g., `version 178.0`). Component versions are seeded for every release component, each starting at increment `0`.
...
- **Version-bound** components are release-specific. They are included in the initial Built Version for tracking (increment `0`), but they are never auto-seeded for successor builds; operators choose them explicitly per deployment cycle.
