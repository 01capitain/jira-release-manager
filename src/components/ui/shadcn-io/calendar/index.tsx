...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  if (showWorkWeekOnly) {
    ...
    for (let i = 0; i < firstWorkingDayOffset; i++) {
      addCell(createBlankCell(`pad-start-${i}`));
    }
...
export const CalendarHeader = ({ className }: CalendarHeaderProps) => {
  const { locale, startDay, visibleDays } = useContext(CalendarContext);

  // Memoize days data to avoid recalculating date formatting
  const daysData = useMemo(() => {
    return daysForLocale(locale, startDay, visibleDays);
  }, [locale, startDay, visibleDays]);

  return (
    <div
      className={cn(
        "grid flex-grow",
        visibleDays === 5 ? "grid-cols-5" : "grid-cols-7",
        className,
      )}
    >
      {daysData.map((day) => (
