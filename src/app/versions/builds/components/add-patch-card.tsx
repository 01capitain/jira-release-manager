...
import { useCreatePatchMutation } from "../api";

type Phase = "idle" | "loading" | "success";

export default function AddPatchCard({
  versionId,
  onCreated,
}: {
  versionId: string;
  onCreated?: (item: PatchDto) => void;
}) {
  const { status } = useAuthSession();
  const { login, isLoggingIn, error: loginError } = useDiscordLogin();
  const createMutation = useCreatePatchMutation();
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const parsed = PatchCreateSchema.safeParse({ name, versionId });
    if (!parsed.success) {
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to create patch. Please try again.";
      setError(message || "Failed to create patch. Please try again.");
    }
  }
        ...
        {status !== "authenticated" ? (
          <CardContent className="min-h-72 p-6">
            <div className="flex h-44 flex-col items-center justify-center gap-3 text-center">
              <div className="text-sm text-neutral-600 dark:text-neutral-300">
                You need to be signed in to add a patch.
              </div>
          ...
          >
            <div className="flex flex-col items-center justify-center text-neutral-400 transition-transform group-hover:scale-105 group-hover:text-neutral-500 dark:text-neutral-500 dark:group-hover:text-neutral-300">
              ...
              <span className="mt-2 text-sm">New Patch</span>
            </div>
          </button>
        ) : (
          <CardContent className="min-h-72 p-6">
            {phase === "success" ? (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-lg font-semibold">New Patch</h3>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">
                      Enter a name to create a patch.
                    </p>
                  </div>
                  <Button
                    type="button"
                ...
                <div className="space-y-2">
                  <Label htmlFor={`patch-name-${versionId}`}>Name</Label>
                  <Input
                    id={`patch-name-${versionId}`}
