...
export const jiraRouter = createTRPCRouter({
  ...
  getCredentials: protectedProcedure.query(
    async ({ ctx }: { ctx: RouterContext }) => {
  ...
  fetchVersions: protectedProcedure
    ...
    .query(
      async ({
        ...
        ctx: RouterContext;
        input: z.infer<typeof JiraFetchVersionsInputSchema>;
      }) => {
  ...
  canSync: protectedProcedure.mutation(
    async ({ ctx }: { ctx: RouterContext }) => {
      const baseUrl = env.JIRA_BASE_URL;
  ...
  canSyncQuick: protectedProcedure.query(
    async ({ ctx }: { ctx: RouterContext }) => {
      const baseUrl = env.JIRA_BASE_URL;
      const projectKey = env.JIRA_PROJECT_KEY;
      const credentialModel = getDelegate(ctx.db, "jiraCredential");
      ...
      if (!token)
        return { ok: false as const, reason: "Missing user API token" };
      return { ok: true as const };
    },
  ),
